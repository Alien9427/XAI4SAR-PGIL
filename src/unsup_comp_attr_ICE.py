from network import ResNet18_TSX_Comp
import numpy as np
import torch
...
def load_pretrained_model(config):
    ...
    if config['iscontinue']:
        model = ResNet18_TSX_Comp(topic_num)
...
def train(config):
    ...
    model = ResNet18_TSX_Comp(config['topic_num'])  # retrain
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='unsup_comp_attr')
    ...
    parser.add_argument('--save_model_path', default='../model/ICE_comp_soft_topic175_retrain_')
