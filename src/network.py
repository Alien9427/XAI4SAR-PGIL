...
class ResNet_Comp(nn.Module):

    def __init__(self, block, layers, num_topic):
        self.inplanes = 64
        super(ResNet_Comp, self).__init__()
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1,1)) # 8 for input of ICE 256*256, 4 for input of Urban 100*100
        self.comp_fc = nn.Sequential(
                                nn.Conv2d(256 * block.expansion, 512, kernel_size=3, stride=2),
                                nn.BatchNorm2d(512, momentum=0.5),
                                nn.ReLU(inplace=True),
        )
        self.comp_layer = nn.Linear(512, num_topic)
